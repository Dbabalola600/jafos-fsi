...
export default function Credit() {
    ...
    const trans: FormEventHandler<HTMLFormElement> = async (e) => {
        ...
        const respone = await fetch("/api/student/transactions/credit", { method: "POST", body: JSON.stringify(body) })
            .then(async res => {
                if (res.status == 200) {
                    const FluttTrans = await fetch("/api/FLUTTER/transfer/chargeCard")
                        .then(res => {
                            if (res.status === 200) {
