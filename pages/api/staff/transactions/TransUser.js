...
export default async function TransUser(req, res) {
    if (req.method === "POST") {
        ...
        const reciever = await Staff.find({ staffid: rec })


        if (reciever[0].staffid === undefined) {
            return res.status(240).json({ message: "not a user" })
        } else {
            if (sender.staffid === reciever[0].staffid) {
                ...
                if (pin === sender.pin) {
                    if (sender.account_bal > amt) {
                        ...
                        const reciever_bal = await Staff.findById(reciever[0]._id).updateOne({ account_bal: new_reciever_bal })
